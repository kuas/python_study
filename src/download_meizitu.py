...
def getPicURLs(html):
    ...
    #http://pic.meizitu.com/wp-content/uploads/2015a/01/14/01.jpg
    reg = r'pic.meizitu.com/wp-content/uploads/\d+\S/\d+/\d+/\d+\.jpg'
    picURLs = re.findall(reg, html)
    urls = []
    for picurl in picURLs:
        if picurl not in urls:
            urls.append("http://%s"%(picurl))
    return urls
